...
local function abbreviateString(str, maxwidth, ellipsis_char)
	if vim.fn.strchars(str) > maxwidth then
		str = vim.fn.strcharpart(str, 0, maxwidth)
...
function lspkind.cmp_format(opts)
	...
	if
		not opts.maxwidth
		or type(opts.maxwidth) == "number"
		or type(opts.maxwidth) == "function"
	then
		opts.maxwidth = {
			abbr = opts.maxwidth,
			menu = opts.maxwidth,
	...
	return function(entry, vim_item)
		...
		if opts.menu ~= nil then
			vim_item.menu = (
				opts.menu[entry.source.name] ~= nil
					and opts.menu[entry.source.name]
				or ""
			)
				.. (
					(opts.show_labelDetails and vim_item.menu ~= nil)
						and vim_item.menu
					or ""
				)
		end
		...
		if opts.maxwidth.menu then
			...
			vim_item.menu =
				abbreviateString(vim_item.menu, maxwidth, ellipsis_char)
		end
		...
		if opts.maxwidth.abbr then
			...
			maxwidth = type(maxwidth) == "function" and maxwidth() or maxwidth
			vim_item.abbr =
				abbreviateString(vim_item.abbr, maxwidth, ellipsis_char)
		end
